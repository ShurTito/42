...
/*   Updated: 2024/12/10 12:22:18 by anttorre         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void Server::checkForChannelsClients(std::vector<std::string> &tmp, int fd)
{
    for (size_t i = 0; i < tmp.size(); i++)
    {
       if (tmp[i][0] == '#')
       {
            ...
            if (!getChannel(tmp[i]))
            {
                senderror("\e[1;33m#" + tmp[i] + WHI, getClient(fd)->getFd(), ": Channel/Nick does not exist.\r\n");
                tmp.erase(tmp.begin() + i);
                i--;
            }
            else if (!getChannel(tmp[i])->getClientInChannel(getClient(fd)->getNickName()))
            {
                senderror(CYA + getClient(fd)->getNickName(), "\e[1;33m#" + tmp[i] + WHI, getClient(fd)->getFd(), ": Cannot send to channel.\r\n");
                tmp.erase(tmp.begin() + i);
                i--;
            }
            else
